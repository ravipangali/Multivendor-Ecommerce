...
@section('content')
    <div class="col-12">
        <form action="{{ route('admin.payment-methods.update', $paymentMethod->id) }}" method="POST" id="paymentMethodForm">
            @csrf
            @method('PUT')
            ...
            @if (request('user_role'))
                <input type="hidden" name="user_role" value="{{ request('user_role') }}">
            @endif
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select @error('type') is-invalid @enderror" id="type" name="type"
                                ...
                                <option value="bank_transfer"
                                    {{ old('type', $paymentMethod->type) == 'bank_transfer' ? 'selected' : '' }}>
                                    <i class="bi bi-bank"></i> Bank Transfer
                                </option>
                                <option value="esewa" {{ old('type', $paymentMethod->type) == 'esewa' ? 'selected' : '' }}>
                                    <i class="bi bi-phone"></i> eSewa
                                </option>
                                <option value="khalti"
                                    {{ old('type', $paymentMethod->type) == 'khalti' ? 'selected' : '' }}>
                                    <i class="bi bi-phone"></i> Khalti
                                </option>
                                <option value="cash" {{ old('type', $paymentMethod->type) == 'cash' ? 'selected' : '' }}>
                                    <i class="bi bi-cash"></i> Cash
                                </option>
                                <option value="other"
                                    {{ old('type', $paymentMethod->type) == 'other' ? 'selected' : '' }}>
                            ...
                            @enderror
                            <small class="form-text text-muted">Choose the type of payment method</small>
                        </div>

                        <div class="col-md-6 mb-3">
                            <label for="title" class="form-label">Title/Nickname <span
                                    class="text-danger">*</span></label>
                            <input type="text" class="form-control @error('title') is-invalid @enderror" id="title"
                                name="title" value="{{ old('title', $paymentMethod->title) }}"
                                ...
                                <div class="col-6">
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" id="is_default" name="is_default"
                                            value="1"
                                            {{ old('is_default', $paymentMethod->is_default) == '1' ? 'checked' : '' }}>
            ...
            <div id="bank_fields" class="card mb-3 {{ $paymentMethod->type != 'bank_transfer' ? 'd-none' : '' }}">
                <div class="card-header bg-light">
                    <h6 class="card-title mb-0">
            ...
            <div id="mobile_fields"
                class="card mb-3 {{ !in_array($paymentMethod->type, ['esewa', 'khalti']) ? 'd-none' : '' }}">
                <div class="card-header bg-light">
                    <h6 class="card-title mb-0">
                    ...
                    <div class="row">
                        <div class="col-12 mb-3">
                            ...
                            <textarea class="form-control @error('notes') is-invalid @enderror" id="notes" name="notes" rows="4"
                                placeholder="Add any additional information about this payment method...">{{ old('notes', $paymentMethod->notes) }}</textarea>
                            @error('notes')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                ...
                <div class="card-body">
                    <div class="d-flex justify-content-end gap-2">
                        <button type="submit" class="btn btn-primary">
                            <i class="align-middle" data-feather="save"></i> Update Payment Method
                        </button>
                        @if (request('user_id') && request('user_role'))
                            @if (request('user_role') == 'customer')
                                <a href="{{ route('admin.customers.show', request('user_id')) }}" class="btn btn-light">
                                    <i class="align-middle" data-feather="x"></i> Cancel
                                </a>
                            @elseif(request('user_role') == 'seller')
                                <a href="{{ route('admin.sellers.show', request('user_id')) }}" class="btn btn-light">
                                    <i class="align-middle" data-feather="x"></i> Cancel
                                </a>
                            @else
