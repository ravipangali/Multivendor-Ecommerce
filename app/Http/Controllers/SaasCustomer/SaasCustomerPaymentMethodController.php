...
class SaasCustomerPaymentMethodController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        return view('saas_customer.saas_payment_methods.saas_index', compact('paymentMethods'));
    }
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'details.branch_name' => 'required_if:type,bank_transfer|nullable|string|max:255',
            'details.account_name' => 'required_if:type,bank_transfer|nullable|string|max:255',
            'details.account_number' => 'required_if:type,bank_transfer|nullable|string|max:255',
            'details.esewa_id' => 'required_if:type,esewa|nullable|string|max:255',
            'details.khalti_id' => 'required_if:type,khalti|nullable|string|max:255',
        ...
        try {
            ...
            SaasPaymentMethod::create([
                'user_id' => Auth::id(),
                'title' => $this->generatePaymentMethodTitle($request->type, $request->details),
                'type' => $request->type,
    ...
    /**
     * Generate a title for the payment method based on its type and details.
     */
    private function generatePaymentMethodTitle($type, $details)
    {
        switch ($type) {
            case 'bank_transfer':
                $accountNumber = $details['account_number'] ?? '';
                return 'Bank Transfer (...'.substr($accountNumber, -4).')';
            case 'esewa':
                $esewaId = $details['esewa_id'] ?? '';
                return 'eSewa (...'.substr($esewaId, -4).')';
            case 'khalti':
                $khaltiId = $details['khalti_id'] ?? '';
                return 'Khalti (...'.substr($khaltiId, -4).')';
            default:
